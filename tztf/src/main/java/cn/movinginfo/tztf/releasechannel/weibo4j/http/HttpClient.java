package cn.movinginfo.tztf.releasechannel.weibo4j.http;
...
import cn.movinginfo.tztf.releasechannel.weibo4j.model.Configuration;
import cn.movinginfo.tztf.releasechannel.weibo4j.model.MySSLSocketFactory;
...
public class HttpClient implements java.io.Serializable {
	 ...
	 */
	public Response multPartURL(String url, PostParameter[] params,
			ImageItem[] items, String token) throws WeiboException {
		PostMethod postMethod = new PostMethod(url);
		try {
			Part[] parts = null;
			if (params == null) {
				...
				parts = new Part[params.length + items.length];
			}
			if (params != null) {
				int i = 0;
				for (PostParameter entry : params) {
