	...
	return OpenLayers.Class(Draw, {
		...
		eventListeners: {
			featureclick: function(e) {
				...
				//t.box.addPopup(t.data.name, t.data.type, data, info.isForecast,info.publisher);
		 ...
		 * @return {object} 生成的点
		 */
		add: function (data, typhoon, isForecast, publisher) {
		 ...
		 */
		adds: function(data, typhoon, isForecast,publisher) {
			var i, ll, n, arr = [];
			for (i = 0, ll = data.length; i < ll; i++) {
				arr[i] = this.add(data[i], typhoon, isForecast,publisher);
			}
			return arr;
		},
		/**
