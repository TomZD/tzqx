...
define(function(require) {
    ...
    var TyphoonBox = OpenLayers.Class({
         ...
         * 清空台风路径
         */
        clearTyphoon: function(id) {
        	if(id){
                for (i = 0, l = this.typhoons.length; i < l; i++) {
                    if(id==this.typhoons[i].id){
         ...
         */
        addPopup: function(name, type, point, isForecast) {
            var lon = point.lon,
                lat = point.lat,
                time = point.time,
            ...
            var html = "<div class='typopup'>" +
                "<span>" + name + "</span>" +
                "<ul>" +
                "<li>"+timeTile+"：" + time.substr(0, 2) + "月" + time.substr(2, 2) + "日" + time.substr(4, 2) + "时</li>" +
        ...
        showLevelLengend: function(x, y) {
            if (!this.gis.levelLegend) {
                var arr = [];
                for (var i = 0, l = this.windPowers.length; i < l; i++) {
                    arr.push(this.windPowers[i]);
                }
                this.gis.levelLegend = this.gis.addControl(LegendLevel, x, y, {
                    content: arr,
                    title: "台风图例"
                });
            }
