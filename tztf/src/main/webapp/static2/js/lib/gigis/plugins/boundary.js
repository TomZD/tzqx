...
define(function(require) {
	...
	var formatXML = function(xml) {
		...
		if (!R.length) {
			...
			for (var tempi = 0; tempi < element.length; tempi++) {
				var areaItem = element[tempi];
				for (var t = 0, length = areaItem.attributes.length; t < length; t++) {
					if (areaItem.attributes[t].name == "Coords") {
						R = areaItem.attributes[t];
					}
					if (areaItem.attributes[t].name == "Center") { //为获取选中区域的center
						Region["center"] = areaItem.attributes[t];
					}
				}
	...
	return function(name, xml, style) {
		...
		if(style.isCut){
			this.map.isCut = true; //覆盖层
		}else{
            this.map.isCut = false; //覆盖层
		}
		this.addLayer(layer);
