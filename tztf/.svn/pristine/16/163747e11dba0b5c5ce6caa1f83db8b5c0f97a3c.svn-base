...
import cn.movinginfo.tztf.sys.service.RoleService;
...
public class RoleAction extends MagicAction<Role, RoleService> {
    ...
    public void saveUserRoles(HttpServletResponse response,@RequestParam(value="userId")String userId,@RequestParam(value="roles")String roles) {
        entityService.saveUserRoles(Long.valueOf(userId),roles,this.getCurrentUser().id);
