define(function(require) {
    ...
    return OpenLayers.Class({
        ...
        initialize: function(data, box) {
            ...
            this.id = data.id;
            this.list = data.list?data.list:data.typhoonInfoItems;
        ...
        format: function(data) {
            ...
            list = data.list?data.list:data.typhoonInfoItems;
        ...
        changeTime: function(time) {
            ...
            $(".m-forecasting-model input").prop("checked","checked");
            //删除已有预报线路
        ...
        addForecast: function(publisher) {
            ...
            for (i = index; i >= 0; i--) {
                ...
                var forecastsList = m && m.forecasts;
                if(forecastsList){
                    for(var mm=0,nn=forecastsList.length;mm<nn;mm++){
                        if(forecastsList[mm].name ==publisher){
                            data =forecastsList[mm].points
                        }
                    }
                }
                if (data) {
                    this.forecasts[publisher] = {
                        points: pointLayer.adds(data, this, true),
                        path: pathsLayer.add(
        ...
        addWindring: function(data) {
            this.removeWindring();
            var data7 = (data.lv7!=9999)?data.lv7:null;
            var data10 = (data.lv10!=9999)?data.lv10:null;
            var data12 = (data.lv12!=9999)?data.lv12:null;
            this.windring = this.box.windringLayer.add(data._lon, data._lat, data7, data10, data12);
        },
        /**
         * 删除风圈
         */
