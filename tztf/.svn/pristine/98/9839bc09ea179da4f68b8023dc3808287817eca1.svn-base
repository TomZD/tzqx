...
mapstol.map.events.on({
    ...
    "click": function(e) {
        ...
        var ll = gis.map.tanslateLonLat(lonlat, true);
        if($('input[name="model.lot"]:visible').length===1 && $('input[name="model.lat"]:visible').length===1) {
        	
        	$('input[name="model.lot"]').val(parseFloat(Math.round(ll.lon * 1000) / 1000).toFixed(3));
        	$('input[name="model.lat"]').val(parseFloat(Math.round(ll.lat * 1000) / 1000).toFixed(3));
        	$.showSuccess("已获取坐标！");
        }
    }
});
...
//             $("#alarmText").append(obj.signalName+"实况预警信息:");
//    		 $("#alarm_time").append(obj.publishTime.toString().replace('T',' '));
//    		 $("#signal_mean").append(obj.signalMean);
//             $("#defence").append(obj.defence);
//             $("#alarm_div").show();
             
             var html = 
            	  '<div class="warn_block alarm" style="z-index: 2000; display: block;" id="alarm_div">'
		         +	'<div class="wb_tit"><i></i><tt id="alarmText">' + obj.signalName + '实况预警信息:</tt><a class="close_warn">×</a></div>'
		         +	'<span id="alarm_time">' + obj.publishTime.toString().replace('T',' ') + '</span>'
		         +	'<p id="signal_mean">' + obj.forecast
		         +	'</p>'
		         +'</div>';
            	 
             pop = mapstol.addPopup(c.longtitude, c.latitude + 0.01, html, 3, 3, true, function(){});
...
//             $("#eventText").append(obj.type+name);
//             $("#wb_tit_id").append(obj.title);
//             $("#wb_tit_time").append(obj.time.toString().replace('T',' '));
//             $("#fbcontent").append(mean);
//             $("#en_div").show();
//             
             var html = 
           	  '<div class="warn_block event" style="z-index: 2000; display: block;" id="alarm_div">'
		         +	'<div class="wb_tit"><i></i><tt id="alarmText">' + obj.type+name + '预警信息:</tt><a class="close_warn">×</a></div>'
		         +	'<p id="signal_mean">' + obj.title
		         +	'</p>'
		         +	'<p id="defence">' + mean
		         +	'</p>'
		         +'</div>';
            ...
            $('.close_warn').click(function(){
    		 ...
    		 var reason ="<b>发布原因:</b>"+ obj.reason;
//    		 console.log(mean);
//    		 $(".snow_warn").remove();
//    		 $("#notice_time").empty();
//             $("#notcontent").empty();
//             $(".alarm").css('display','none');
//             $(".event").css('display','none');
//    		 $("#notice_time").append(obj.publishTime.toString().replace('T',' '));
//    		 $("#notcontent").append(mean);
//             $("#notice_div").show();
    		 
    		 var html = 
           	  	  '<div class="warn_block notice" style="z-index: 2000; display: block;" id="alarm_div">'
		         +	  '<div class="wb_tit"><i></i><tt id="alarmText">' + obj.title + '</tt><a class="close_warn">×</a></div>'
		         +	  '<span id="alarm_time">' + obj.publishTime.toString().replace('T',' ') + '</span>'
		         +	  '<p id="defence">' + reason
		         +	  '</p>'
		         +'</div>';
    ...
    $.ajax({
        url:'/outter/index!setPointsList.action',
        success: function (list) {
            var length = list.length;
            for(var i=0;i<length;++i){
            	var data = list[i];
                var isRecord=true;//允许标点记录
            	setPoints.add(data.point, data.type, data.popup_name,data.icon, true,isRecord);
            }
        },
        error: function (ex) {
