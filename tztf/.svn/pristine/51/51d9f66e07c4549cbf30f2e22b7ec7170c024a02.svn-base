...
public interface PermissionMapper extends Mapper<Permission> {
    ...
    @Select("SELECT * FROM sys_permission where id in (select permission_id from sys_r_role_permission where valid=1 and role_id in (select role_id from sys_r_user_role where valid=1 and user_id=#{value})) order by indexes ASC")
